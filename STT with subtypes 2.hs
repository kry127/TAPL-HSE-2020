...
addMissingTypes :: [(Symb, Type)] -> [(Symb, Type)] -> [(Symb, Type)]
addMissingTypes (e@(name, _):from) to = addMissingTypes from $ maybe (e:to) (const to) (lookup name to)
addMissingTypes _ to = to
-- конец вспомогательных функций

(<:) :: Type -> Type -> Bool
_ <: Top = True
Top <: _ = False
(a :-> b) <: (aSub :-> bSuper) = (aSub <: a) && (b <: bSuper)
-- будем сразу строить монаду, чтобы не городить list comprehension
l1@(TRcd _) <: l2@(TRcd _) = foldr (&&) True $ mkListBools l1 l2
  where
    mkListBools::Type -> Type -> [Bool]
    mkListBools (TRcd l1) (TRcd l2) = do
      (name, a) <- l2
      let bl = [b' | (name', b') <- l1, name == name']
      if length bl /= 1 then return False
      else do
        let [b] = bl
        return (b <: a)
x   <: y   | x == y = True
...
x   \/ y   | x <: y = y
x   \/ y   | y <: x = x
(a:->b) \/ (c:->d) = maybe Top (:->(b \/ d)) (a /\ c)
l1@(TRcd _) \/ l2@(TRcd _) = TRcd $ mkListUnions l1 l2
  where
    mkListUnions::Type -> Type -> [(Symb, Type)]
    mkListUnions (TRcd l1) (TRcd l2) = [(name, (b \/ a)) | (name, a) <- l2, (name', b) <- l1, name == name']
_   \/ _         = Top
...
x   /\ y   | y <: x = Just y
(a:->b) /\ (c:->d) = ((a \/ c):->) <$> (b /\ d)
t1@(TRcd list1) /\ t2@(TRcd list2) = TRcd <$> (addMissingTypes list1) <$> (addMissingTypes list2) <$> sequenceA (mkListIntersects t1 t2)
  where
    mkListIntersects::Type -> Type -> [Maybe (Symb, Type)]
    mkListIntersects (TRcd l1) (TRcd l2) = [((,) name) <$> (b /\ a) | (name', b) <- l1, (name, a) <- l2, name == name']
_   /\ _         = Nothing
